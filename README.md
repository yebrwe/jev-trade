...
리스크%는 **손절 시 총손실**(가격 손실 + 왕복 수수료 + 슬리피지 + 예상 펀딩) 기준입니다. 5분봉 손절 거리 0.33%에서는
비용이 예산의 약 37%를 차지하므로, 가격 손실만으로 수량을 잡으면 실제 손실이 예산의 1.6배가 됩니다.
...
거래소 통신 없이 `logs/` 파일만 읽어 실시간 잔고·포지션·판단을 보여줍니다. 10초마다 자동 갱신됩니다.

```bash
python -m jev_trade dashboard            # http://127.0.0.1:8787
```

- 총자산(지갑 + 미실현), 오늘·시작 이후 손익, 고점 대비 낙폭, 거래 수·승률
- 현재 포지션(방향, 수량, 진입가, 미실현, 배수, 증거금, 손절·익절, 청산가, 보유 시간)
- 자산 곡선(마우스 올리면 시각·총자산·지갑·BTC 가격), 최근 판단 40개(롱/홀드/숏 확률 막대, 확신, 배수·리스크, 사유), 청산된 거래
- 관점 요약과 뉴스 감시 상태, 현재 설정값

봇은 청산이 일어날 때마다 `logs/trades.jsonl`에 기록합니다. 거래소가 손절·익절로 청산한 경우는 다음 사이클에
감지해 현재가 기준 추정 손익으로 기록하고 "(추정)"으로 표시합니다.
